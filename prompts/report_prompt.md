...
### 2. Single-cell and molecular feature analysis (includes pathway analysis)
...
reduced relative to control (add 1-2 lines on the likely biological meaning);
and the precision marker genes. Then add a short "Pathway analysis" paragraph
interpreting the activated biological processes in {pathways.activated} and the
suppressed gene program in {pathways.suppressed_genes}. IMPORTANT: do NOT invent
pathway names, q-values, or gene lists — a pathway table is inserted automatically
below your text; only interpret what is in the payload. Reference the UMAP,
composition bar plot, and marker heatmap figures.
...
the top dual-gene (combination) targets. Interpret which target and which
combination are most actionable FOR THIS PATIENT and why. Then add a short
"Mechanism of action" sentence explaining WHY the top target works, drawing on
{mechanism.narrative} — the affected gene network, the enriched pathway/function,
and the implicated transcription factors. IMPORTANT: do NOT invent or name any
drugs, pathways, TFs, or genes — a locked drug table and a mechanism block
(affected genes -> pathway + TF enrichment) are inserted automatically below your
text; only interpret what the payload provides. Reference the perturbation bar
plot and perturbation UMAP figures.
